...
CodeMirror.defineMode("smartymixed", function(config) {
  ...
  if (config.hasOwnProperty("leftDecxcter")) {
    settings.leftDecxcter = config.leftDecxcter;
  }
  if (config.hasOwnProperty("rightDecxcter")) {
    settings.rightDecxcter = config.rightDecxcter;
  }

  function reEsc(str) { return str.replace(/[^\s\w]/g, "\\$&"); }

  var reLeft = reEsc(settings.leftDecxcter), reRight = reEsc(settings.rightDecxcter);
  var regs = {
  ...
  var parsers = {
    html: function(stream, state) {
      ...
      if (!state.inLiteral && stream.match(regs.htmlHasLeftDelimeter, false) && htmlTagName === null) {
        ...
        return helpers.maybeBackup(stream, settings.leftDecxcter, smartyMode.token(stream, state.localState));
      } else if (!state.inLiteral && stream.match(settings.leftDecxcter, false)) {
        state.tokenize = parsers.smarty;
        state.localMode = smartyMode;
        state.localState = smartyMode.startState(htmlMixedMode.indent(state.htmlMixedState, ""));
        return helpers.maybeBackup(stream, settings.leftDecxcter, smartyMode.token(stream, state.localState));
    ...
    smarty: function(stream, state) {
      if (stream.match(settings.leftDecxcter, false)) {
        if (stream.match(regs.smartyComment, false)) {
          return helpers.chain(stream, state, parsers.inBlock("comment", "*" + settings.rightDecxcter));
        }
      } else if (stream.match(settings.rightDecxcter, false)) {
        stream.eat(settings.rightDecxcter);
      ...
      return helpers.maybeBackup(stream, settings.rightDecxcter, smartyMode.token(stream, state.localState));
    },
