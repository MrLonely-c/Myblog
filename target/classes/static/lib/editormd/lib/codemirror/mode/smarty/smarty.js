...
CodeMirror.defineMode("smarty", function(config) {
  ...
  var parsers = {
    ...
    tokenizer: function(stream, state) {
      if (stream.match(settings.leftDecxcter, true)) {
        if (stream.eat("*")) {
          ...
          if (settings.smartyVersion === 3 && settings.leftDecxcter === "{" && (isEol || isFollowedByWhitespace)) {
            state.depth--;
            return null;
          } else {
    ...
    smarty: function(stream, state) {
      if (stream.match(settings.rightDecxcter, true)) {
